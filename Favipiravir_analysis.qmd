...
```{r}
intervention = 'Favipiravir' # prefix of analysis file
...
platcov_dat = platcov_dat %>% group_by(ID, Timepoint_ID) %>%
  mutate(daily_VL = mean(log10_viral_load),
         ...
         Variant = factor(Variant,levels=c('Delta','BA.1','BA.2','BA.3','BA.4','BA.5'))) %>%
  ungroup() %>%
...
vals = as.POSIXct(seq.Date(from = as.Date('2021-10-01'), 
                                 to = as.Date('2022-10-01'),length.out = 5))
axis(1, at = vals, labels = lubridate::as_date(vals))
...
plot(xx$Weight, xx$t_12_med, panel.first=grid(),
     ylim = c(0,50), col = as.character(xx$trt_color), pch =16,
     xlab = 'Patient weight (kg)', ylab=expression('t'[1/2] ~ ' (hours)'))
ind=xx$Trt=='Favipiravir'
cor.test(xx$Weight[ind], xx$t_12_med[ind])
```
