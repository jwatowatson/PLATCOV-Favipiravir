...
make_slopes_plot = function(stan_out, 
  ...
  plot(1:nrow(data_summary),data_summary$t_12_med,
       col=trt_cols[data_summary$Trt],  xaxt='n',
       ylim=my_lims, panel.first=grid(), xlab='', 
       pch=15, ylab=expression('t'[1/2] ~ ' (hours)'))
  for(i in 1:nrow(data_summary)){
    lines(rep(i,2),
          c(data_summary$t_12_low[i],
            data_summary$t_12_up[i]),
          col=trt_cols[data_summary$Trt[i]])
  }
  ...
  for(trt in unique(data_summary$Trt)){
    ...
    abline(h = median(data_summary$t_12_med[ind]), col=trt_cols[trt],lty=2,lwd=2)
  }
